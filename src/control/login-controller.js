...
module.exports.LoginController = class LoginController {
    ...
    async loadLoginPage(req, res, notification = '') {
    ...
    async attemptToLogin(req, res) {
        ...
        if (!loginModel.hasUsername()) {
            await this.loadLoginPage(req, res, 'Username is empty');
            return;
        }

        if (!loginModel.hasPassword()) {
            await this.loadLoginPage(req, res, 'Password is empty');
            return;
        }

        if (!loginModel.isLogged(true)) {
            await this.loadLoginPage(req, res, 'Username and/or password is incorrect');
            return;
        }
    ...
    listenGetEvent() {
        this.app.get('/login', async (req, res) => {
            if (this.sessionService.isLogged(req))
                ...
                await this.loadLoginPage(req, res)
        });
    }
